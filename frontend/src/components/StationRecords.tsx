...
function TxRow({ record, onClick, onMouseEnter }: TxRowProps) {
  ...
  return (
    ...
    >
      <td className="px-4 py-3 text-sm font-mono text-storm-800 dark:text-storm-200 whitespace-nowrap">
        {formatDateTime(record.timestamp)}
      </td>
      <td className="px-4 py-3 text-sm font-mono">
        {record.blockchain.txid
          ? <span className="text-sunrise-400">{truncateTxid(record.blockchain.txid)}</span>
          : <span className="text-storm-400 dark:text-storm-600">—</span>}
      </td>
      <td className="px-4 py-3 text-sm font-mono text-storm-800 dark:text-storm-200">
...
export function StationRecords() {
  ...
  if (error && !recordsData) {
    return (
      <div className="text-center py-24">
        ...
        <Link to="/explorer" className="mt-4 inline-block text-sunrise-400 hover:underline text-sm">
  ...
  return (
    <div className="space-y-4">
      <div>
        <Link to="/explorer" className="text-sunrise-400 hover:underline text-sm">
          &larr; Back to dashboard
        </Link>
      </div>

      {/* Station Header */}
      <div className="bg-white dark:bg-storm-850 border border-storm-200 dark:border-storm-700 rounded-lg px-6 py-5">
        <div>
          <h1 className="text-lg font-semibold text-storm-950 dark:text-storm-100">
            Station {stationId}{station?.name ? ` — ${station.name}` : ''}
          </h1>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
            {station?.location && (
              <span className="text-sm text-storm-600 dark:text-storm-400">{station.location}</span>
            )}
        ...
        <div className="flex gap-8 mt-4 pt-4 border-t border-storm-200 dark:border-storm-700">
          <div>
            <p className="text-xs text-storm-400 dark:text-storm-400 uppercase tracking-wide">Total Records</p>
            <p className="text-lg font-semibold text-storm-950 dark:text-storm-100 mt-0.5">
              {pagination?.total?.toLocaleString() ?? station?.txRecords?.toLocaleString() ?? '—'}
            </p>
          </div>
          <div>
            <p className="text-xs text-storm-400 dark:text-storm-400 uppercase tracking-wide">Last Block Written</p>
            <p className="text-lg font-semibold text-storm-950 dark:text-storm-100 mt-0.5">
      ...
      <div className="bg-white dark:bg-storm-850 border border-storm-200 dark:border-storm-700 rounded-lg overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-storm-100 dark:bg-storm-900/60">
                ...
                <th className="px-4 py-3 text-xs font-medium text-storm-600 dark:text-storm-400 uppercase tracking-wide">TxID</th>
                <SortableTh label="Block Height" sortKey="blockHeight" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} />
                <th className="px-4 py-3 text-xs font-medium text-storm-600 dark:text-storm-400 uppercase tracking-wide">QC Status</th>
            ...
            <tbody className={isFetching && recordsData ? 'opacity-50 transition-opacity duration-150' : 'transition-opacity duration-150'}>
              {records.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-storm-400 dark:text-storm-400">No records found</td>
                </tr>
              ) : (
        ...
        {pagination && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-storm-200 dark:border-storm-700">
            ...
            <div className="flex items-center gap-3 text-sm text-storm-600 dark:text-storm-400">
              {isFetching && (
                <div className="w-3.5 h-3.5 rounded-full border-2 border-sunrise-400 border-t-transparent animate-spin" />
              )}
              <span>
                Page {pagination.page} of {pagination.totalPages}
                <span className="ml-4 text-storm-400 dark:text-storm-400">Total: {pagination.total.toLocaleString()}</span>
              </span>
            </div>
            <button
